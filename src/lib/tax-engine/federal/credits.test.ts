...
describe('calculateAgeCredit', () => {
  it('returns full $9,028 for someone born 1955-06-15 with $30,000 income', () => {
    // Age 70 on Dec 31, 2025; income below clawback start of $44,325
    expect(calculateAgeCredit(30_000, '1955-06-15')).toBe(9_028);
  });

  it('reduces the age credit for $80,000 income (born 1955-06-15)', () => {
    // reduction = round((80000 - 44325) * 0.15) = round(5351.25) = 5351.25
    // credit = 9028 - 5351.25 = 3676.75
    expect(calculateAgeCredit(80_000, '1955-06-15')).toBe(3_676.75);
  });
...
describe('calculateCanadaEmploymentCredit', () => {
  it('returns $1,368 when employed', () => {
    expect(calculateCanadaEmploymentCredit(true)).toBe(1_368);
  });
...
describe('calculateMedicalExpenseCredit', () => {
  it('eligible = $5,000 - min($2,759, 3% of $60,000) = $5,000 - $1,800 = $3,200', () => {
    // 3% of 60000 = 1800 < 2759, so threshold = 1800
    expect(calculateMedicalExpenseCredit(5_000, 60_000)).toBe(3_200);
  });

  it('uses the $2,759 flat threshold when 3% of income exceeds it', () => {
    // 3% of 120000 = 3600 > 2759, so threshold = 2759
...
describe('calculateDonationCredit', () => {
  ...
  it('applies 33% on $200+ amount exactly at the top-bracket threshold', () => {
    // taxableIncome = 253415 (just over 253414 threshold)
    // (200 × 0.15) + (800 × 0.33) = 30 + 264 = 294
...
describe('calculateTotalFederalCredits', () => {
  it('computes correct totals for an employed person at $65,000 income', () => {
    ...
    // Employment credit
    expect(result.creditAmounts.employmentAmount).toBe(1_368);

    // Medical: threshold = min(2759, 65000×0.03=1950) = 1950; 800 < 1950 → 0
    expect(result.creditAmounts.medicalAmount).toBe(0);

    // totalCreditAmount = 16129 + 0 + 3354 + 1077 + 1368 + 0 + 0 + 0 + 0 + 0 = 21928
    expect(result.totalCreditAmount).toBe(21_928);

    // Donation credit: (200×0.15) + (300×0.29) = 30 + 87 = 117
    expect(result.donationCreditValue).toBe(117);

    // totalCreditValue = 21928 × 0.15 + 117 = 3289.20 + 117 = 3406.20
    expect(result.totalCreditValue).toBe(3_406.2);
  });

  it('includes age credit and disability for a senior with disability', () => {
    // Born 1955-01-01 (70 in 2025), income $40,000 (below age clawback start)
    const result = calculateTotalFederalCredits({
      netIncome: 40_000,
    ...
    // Age credit: 9028 (income 40000 < 44325)
    expect(result.creditAmounts.ageAmount).toBe(9_028);

    // Pension: min(1200, 2000) = 1200
    expect(result.creditAmounts.pensionAmount).toBe(1_200);

    // Medical: threshold = min(2759, 40000×0.03=1200) = 1200; 3000 - 1200 = 1800
